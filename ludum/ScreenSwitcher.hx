...
class ScreenSwitcher {		
	public static inline function checkForBoundaryCrossing(lastPosition:Vector3, currentPosition:Vector3):Bool {
	...
	public static inline function getScreenIndices(position:Vector3):Vector2 {
		return new Vector2(Math.round(position.x / (Main.GAME_VIEWPORT_WIDTH)), Std.int(position.y / (Main.GAME_VIEWPORT_HEIGHT)));
	}
	
	public static inline function getNearestCameraClampX(position:Vector3):Float {
